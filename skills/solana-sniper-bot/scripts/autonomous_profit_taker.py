#!/usr/bin/env python3
"""Autonomous Profit Taker v2.3 - Moonbag Tracking & Volume Decay."""
...
async def loop():
    log.info("Profit Taker v2.3 ACTIVE.")
    while True:
        try:
            ...
            for mint, data in positions.items():
                ...
                prev_vol = data.get("prev_vol", curr_vol)
                ...
                if not data.get("sold_initial"):
                    ...
                    if peak_roi >= TP_MULT and (peak_roi - roi) >= TRAILING_DROP:
                        if execute_exit(mint, 80, "TRAILING PROFIT"):
                            ...
                            updated = True
                    elif roi >= (TP_MULT + 1.0): # 2.3x hard TP
