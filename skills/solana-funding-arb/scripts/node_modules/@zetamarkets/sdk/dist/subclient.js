...
class SubClient {
     ...
     */
    static load(asset, parent, connection, user, fetchedAccs, callback = undefined, throttle = false) {
        let subClient = new SubClient(asset, parent);
        let [marginAccountAddress, _marginAccountNonce] = utils.getMarginAccount(exchange_1.exchange.programId, subClient._subExchange.zetaGroupAddress, user);
        let [spreadAccountAddress, _spreadAccountNonce] = utils.getSpreadAccount(exchange_1.exchange.programId, subClient._subExchange.zetaGroupAddress, user);
        ...
        subClient._marginAccountSubscriptionId = connection.onAccountChange(subClient._marginAccountAddress, async (accountInfo, context) => {
            ...
            if (callback !== undefined) {
                callback(asset, events_1.EventType.USER, {
        ...
        if (fetchedAccs[0] != null) {
            subClient._marginAccount = fetchedAccs[0];
        ...
        if (fetchedAccs[1] != null) {
            subClient._spreadAccount = fetchedAccs[1];
            subClient.updateSpreadPositions();
        }
        else {
        ...
        if (exchange_1.exchange.clockTimestamp >
            ...
            try {
                ...
                if (this._callback !== undefined) {
                    this._callback(this.asset, events_1.EventType.USER, {
    ...
    async deposit(amount) {
        ...
        tx.add(instructions.depositIx(this.asset, amount, this._marginAccountAddress, this._parent.usdcAccountAddress, this._parent.provider.wallet.publicKey, this._parent.whitelistDepositAddress));
        return await utils.processTransaction(this._parent.provider, tx, undefined, undefined, undefined, this._parent.useVersionedTxs ? utils.getZetaLutArr() : undefined);
    }
    /**
    ...
    async closeMarginAccount() {
        ...
        let txId = await utils.processTransaction(this._parent.provider, tx, undefined, undefined, undefined, this._parent.useVersionedTxs ? utils.getZetaLutArr() : undefined);
        this._marginAccount = null;
        return txId;
    }
    /**
    ...
    async placeOrderAndLockPosition(market, price, size, side, tag = constants.DEFAULT_ORDER_TAG) {
        ...
        if (this._openOrdersAccounts[marketIndex].equals(web3_js_1.PublicKey.default)) {
            ...
            let [initIx, _openOrdersPda] = instructions.initializeOpenOrdersIx(this.asset, market, this._parent.publicKey, this._parent.provider.wallet.publicKey, this.marginAccountAddress);
    ...
    async placeOrder(market, price, size, side, options = types.defaultOrderOptions()) {
        ...
        tx.add(orderIx);
        let txId = await utils.processTransaction(this._parent.provider, tx, undefined, undefined, undefined, this._parent.useVersionedTxs ? utils.getZetaLutArr() : undefined, options.blockhash);
        this._openOrdersAccounts[marketIndex] = openOrdersPda;
        return txId;
    }
    /**
    ...
    async cancelAndPlaceOrder(market, orderId, cancelSide, newOrderPrice, newOrderSize, newOrderSide, options = types.defaultOrderOptions()) {
        ...
        return await utils.processTransaction(this._parent.provider, tx, undefined, undefined, undefined, this._parent.useVersionedTxs ? utils.getZetaLutArr() : undefined, options.blockhash);
    }
    /**
    ...
    async cancelAndPlaceOrderByClientOrderId(market, cancelClientOrderId, newOrderPrice, newOrderSize, newOrderSide, newOptions = types.defaultOrderOptions()) {
        ...
        return await utils.processTransaction(this._parent.provider, tx, undefined, undefined, undefined, this._parent.useVersionedTxs ? utils.getZetaLutArr() : undefined, newOptions.blockhash);
    }
    /**
    ...
    async initializeOpenOrdersAccount(market) {
        ...
        let [initIx, openOrdersPda] = instructions.initializeOpenOrdersIx(this.asset, market, this._parent.publicKey, this._parent.provider.wallet.publicKey, this.marginAccountAddress);
    ...
    async closeOpenOrdersAccount(market) {
        ...
        const [vaultOwner, _vaultSignerNonce] = utils.getSerumVaultOwnerAndNonce(market, constants.DEX_PID[exchange_1.exchange.network]);
        let tx = new web3_js_1.Transaction();
        tx.add(instructions.settleDexFundsIx(this.asset, market, vaultOwner, this._openOrdersAccounts[marketIndex]));
        tx.add(instructions.closeOpenOrdersIx(this.asset, market, this._parent.provider.wallet.publicKey, this.marginAccountAddress, this._openOrdersAccounts[marketIndex]));
        let txId = await utils.processTransaction(this._parent.provider, tx, undefined, undefined, undefined, this._parent.useVersionedTxs ? utils.getZetaLutArr() : undefined);
        this._openOrdersAccounts[marketIndex] = web3_js_1.PublicKey.default;
        return txId;
    }
    updateSubExchange() {
        this._subExchange = exchange_1.exchange.getSubExchange(this._asset);
    }
    /**
    ...
    async closeMultipleOpenOrdersAccount(markets) {
        ...
        for (var i = 0; i < markets.length; i++) {
            ...
            let closeIx = instructions.closeOpenOrdersIx(this.asset, market, this._parent.provider.wallet.publicKey, this.marginAccountAddress, this._openOrdersAccounts[marketIndex]);
    ...
    async forceCancelOrderByOrderId(market, marginAccountToCancel, orderId, side) {
        ...
        let marketIndex = this._subExchange.markets.getMarketIndex(market);
        let openOrdersAccountToCancel = utils.createOpenOrdersAddress(exchange_1.exchange.programId, market, marginAccount.authority, marginAccount.openOrdersNonce[marketIndex]);
        let tx = new web3_js_1.Transaction();
    ...
    async cancelAllOrders() {
        ...
        // Compute is fine.
        let txs = utils.splitIxsIntoTx(this.cancelAllOrdersIxs(), this._parent.useVersionedTxs
            ? constants.MAX_CANCELS_PER_TX_LUT
            : constants.MAX_CANCELS_PER_TX);
        let txIds = [];
        await Promise.all(txs.map(async (tx) => {
            txIds.push(await utils.processTransaction(this._parent.provider, tx, undefined, undefined, undefined, this._parent.useVersionedTxs ? utils.getZetaLutArr() : undefined));
        }));
        return txIds;
    }
    /**
    ...
    async cancelAllOrdersNoError() {
        ...
        // on 4 separate markets
        // Compute is fine.
        let txs = utils.splitIxsIntoTx(this.cancelAllOrdersNoErrorIxs(), this._parent.useVersionedTxs
            ? constants.MAX_CANCELS_PER_TX_LUT
            : constants.MAX_CANCELS_PER_TX);
        let txIds = [];
        await Promise.all(txs.map(async (tx) => {
    ...
    async simulatePositionMovement(movementType, movements) {
        ...
        while (true) {
            ...
            if (event.value.name == "PositionMovementEvent") {
                positionMovementEvent = event.value.data;
                break;
            }
        }
    ...
    updateOpenOrdersAddresses() {
        this._subExchange.zetaGroup.products.map((product, index) => {
            ...
            this._marginAccount.openOrdersNonce[index] !== 0 &&
                ...
                let [openOrdersPda, _openOrdersNonce] = utils.getOpenOrders(exchange_1.exchange.programId, product.market, this._parent.publicKey);
        ...
        this._marginAccount.openOrdersNonce[constants.PERP_INDEX] !== 0 &&
            ...
            let [openOrdersPda, _openOrdersNonce] = utils.getOpenOrders(exchange_1.exchange.programId, this._subExchange.zetaGroup.perp.market, this._parent.publicKey);
