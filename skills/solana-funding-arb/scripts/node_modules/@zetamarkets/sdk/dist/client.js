...
class Client {
    ...
    static async load(connection, wallet, opts = utils.defaultCommitment(), callback = undefined, throttle = false, delegator = undefined, useVersionedTxs = false) {
        ...
        client._usdcAccountAddress = utils.getAssociatedTokenAddress(exchange_1.exchange.usdcMintAddress, owner);
        client._whitelistDepositAddress = undefined;
        client._whitelistTradingFeesAddress = undefined;
        const ACCS_PER_SUBCLIENT = 2;
        const subClientToFetchPromises = exchange_1.exchange.assets
            .map((a) => {
            let marginAccountAddress = utils.getMarginAccount(exchange_1.exchange.programId, exchange_1.exchange.getZetaGroupAddress(a), owner)[0];
            let spreadAccountAddress = utils.getSpreadAccount(exchange_1.exchange.programId, exchange_1.exchange.getZetaGroupAddress(a), owner)[0];
            return [
                exchange_1.exchange.program.account.marginAccount.fetchNullable(marginAccountAddress),
                exchange_1.exchange.program.account.spreadAccount.fetchNullable(spreadAccountAddress),
            ];
        })
            .flat();
        const whitelistDepositAddress = utils.getUserWhitelistDepositAccount(exchange_1.exchange.programId, owner)[0];
        const whitelistTradingFeesAddress = utils.getUserWhitelistTradingFeesAccount(exchange_1.exchange.programId, owner)[0];
        const promiseResults = await Promise.all(subClientToFetchPromises.concat([
            exchange_1.exchange.program.account.whitelistDepositAccount.fetchNullable(whitelistDepositAddress),
            exchange_1.exchange.program.account.whitelistTradingFeesAccount.fetchNullable(whitelistTradingFeesAddress),
        ]));
        if (promiseResults[exchange_1.exchange.assets.length * ACCS_PER_SUBCLIENT] != null) {
            console.log("User is whitelisted for unlimited deposits into zeta.");
            client._whitelistDepositAddress = whitelistDepositAddress;
        }
        if (promiseResults[exchange_1.exchange.assets.length * ACCS_PER_SUBCLIENT + 1] != null) {
            console.log("User is whitelisted for trading fees.");
            client._whitelistTradingFeesAddress = whitelistTradingFeesAddress;
        }
        exchange_1.exchange.assets.forEach((asset, i) => {
            const subClient = subclient_1.SubClient.load(asset, client, connection, owner, [
        ...
        if (callback !== undefined) {
            client._tradeEventV3Listener = exchange_1.exchange.program.addEventListener("TradeEventV3", (event, _slot) => {
                if (client._marginAccountToAsset.has(event.marginAccount.toString())) {
                    callback(client._marginAccountToAsset.get(event.marginAccount.toString()), events_1.EventType.TRADEV3, event);
                }
            });
    ...
    setUseVersionedTxs(useVersionedTxs) {
    ...
    async setReferralData() {
        this.delegatedCheck();
        try {
            let [referrerAccount] = utils.getReferrerAccountAddress(exchange_1.exchange.programId, this.publicKey);
        ...
        try {
            let [referralAccountAddress, _nonce] = utils.getReferralAccountAddress(exchange_1.exchange.programId, this.publicKey);
    ...
    async referUser(referrer) {
        this.delegatedCheck();
        let [referrerAccount] = utils.getReferrerAccountAddress(exchange_1.exchange.programId, referrer);
        try {
            await exchange_1.exchange.program.account.referrerAccount.fetch(referrerAccount);
        }
        catch (e) {
        ...
        let tx = new web3_js_1.Transaction().add((0, program_instructions_1.referUserIx)(this.provider.wallet.publicKey, referrer));
        let txId = await utils.processTransaction(this.provider, tx);
        [this._referralAccountAddress] = utils.getReferralAccountAddress(exchange_1.exchange.programId, this.publicKey);
        this._referralAccount =
    ...
    async migrateFunds(amount, withdrawAsset, depositAsset) {
        ...
        tx.add(instructions.depositIx(depositAsset, amount, depositSubClient.marginAccountAddress, this.usdcAccountAddress, this.provider.wallet.publicKey, this.whitelistDepositAddress));
    ...
    async cancelAllOrders(asset = undefined) {
        if (asset != undefined) {
            ...
            let txs = utils.splitIxsIntoTx(ixs, this.useVersionedTxs
                ? constants.MAX_CANCELS_PER_TX_LUT
                : constants.MAX_CANCELS_PER_TX);
            let txIds = [];
            await Promise.all(txs.map(async (tx) => {
                txIds.push(await utils.processTransaction(this.provider, tx, undefined, undefined, undefined, this.useVersionedTxs ? utils.getZetaLutArr() : undefined));
            }));
            return txIds;
        }
    }
    ...
    getMarginAccountState(asset, pnlExecutionPrice = undefined, pnlAddTakerFees = false) {
        return exchange_1.exchange.riskCalculator.getMarginAccountState(this.getSubClient(asset).marginAccount, pnlExecutionPrice, pnlAddTakerFees);
    }
    ...
    async cancelAllPerpMarketOrders() {
        let tx = new web3_js_1.Transaction();
        for (var asset of exchange_1.exchange.assets) {
            let sc = this.getSubClient(asset);
            if (sc.marginAccount == null ||
                sc.openOrdersAccounts[constants.PERP_INDEX].equals(web3_js_1.PublicKey.default)) {
                continue;
            }
            tx.add(instructions.cancelAllMarketOrdersIx(asset, constants.PERP_INDEX, this.provider.wallet.publicKey, this.getSubClient(asset).marginAccountAddress, this.getSubClient(asset).openOrdersAccounts[constants.PERP_INDEX]));
        }
        return await utils.processTransaction(this.provider, tx, undefined, undefined, undefined, this.useVersionedTxs ? utils.getZetaLutArr() : undefined);
    }
    ...
    async initializeReferrerAlias(alias) {
        ...
        let [referrerAccountAddress] = utils.getReferrerAccountAddress(exchange_1.exchange.programId, this.publicKey);
        try {
    ...
    async claimReferralRewards() {
        this.delegatedCheck();
        let [referralAccountAddress] = utils.getReferralAccountAddress(exchange_1.exchange.programId, this.publicKey);
    ...
    async close() {
        ...
        if (this._tradeEventV3Listener !== undefined) {
            await exchange_1.exchange.program.removeEventListener(this._tradeEventV3Listener);
