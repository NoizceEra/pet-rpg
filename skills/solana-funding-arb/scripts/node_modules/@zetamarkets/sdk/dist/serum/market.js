...
exports.throwIfNull = exports.getMintDecimals = exports.Orderbook = exports.ORDERBOOK_LAYOUT = exports.OpenOrders = exports._OPEN_ORDERS_LAYOUT_V2 = exports.Market = exports.MARKET_STATE_LAYOUT_V3 = void 0;
const buffer_layout_1 = require("buffer-layout");
const layout_1 = require("./layout");
const slab_1 = require("./slab");
const bn_js_1 = __importDefault(require("bn.js"));
const web3_js_1 = require("@solana/web3.js");
const zeta_solana_web3_1 = require("zeta-solana-web3");
...
class Market {
     ...
     */
    static loadFromDecoded(decoded, options = {}, programId, layoutOverride) {
        // Use local clock if Exchange.clockTimestamp isn't set yet
        let nowTs = Math.floor(Date.now() / 1000);
        if (exchange_1.exchange.clockTimestamp != undefined && exchange_1.exchange.clockTimestamp != 0) {
            nowTs = exchange_1.exchange.clockTimestamp;
        }
        decoded.epochStartTs = new bn_js_1.default(nowTs - (nowTs % decoded.epochLength.toNumber()));
        return new Market(decoded, 0, 6, options, programId, layoutOverride);
    }
    get programId() {
    ...
    async updateDecoded(connection) {
        const { owner, data } = throwIfNull(await connection.getAccountInfo(this.address), "Market not found");
        this._decoded = exports.MARKET_STATE_LAYOUT_V3.decode(data);
        return;
    }
    ...
    async loadBidsAndAsks(connection) {
        let [clockInfo, bidsInfo, asksInfo] = await connection.getMultipleAccountsInfo([
            web3_js_1.SYSVAR_CLOCK_PUBKEY,
            this._decoded.bids,
            this._decoded.asks,
        ]);
        let slot = (0, utils_1.getClockData)(clockInfo).slot;
        return {
            slot: slot,
    ...
    loadBidsAndAsksFromData(clockInfo, bidsInfo, asksInfo) {
        return {
            slot: clockInfo.slot,
            bids: Orderbook.decode(this, throwIfNull(bidsInfo).data),
            asks: Orderbook.decode(this, throwIfNull(asksInfo).data),
    ...
    async loadEventQueueHeader(connection) {
        const { data } = throwIfNull(await connection.getAccountInfo(this._decoded.eventQueue));
        return (0, queue_1.decodeEventQueue)(data, undefined, true);
    }
...
async function getMintDecimals(connection, mint) {
    if (mint.equals(new zeta_solana_web3_1.PublicKey("So11111111111111111111111111111111111111112"))) {
        return 9;
    }
