...
class InsuranceClient {
    ...
    static async load(connection, wallet, opts = utils.defaultCommitment()) {
        ...
        let [insuranceDepositAccountAddress, _insuranceDepositAccountNonce] = await utils.getUserInsuranceDepositAccount(exchange_1.exchange.programId, wallet.publicKey);
    ...
    async reloadInsuranceDepositAccount() {
        let [insuranceDepositAccountAddress, _insuranceDepositAccountNonce] = utils.getUserInsuranceDepositAccount(exchange_1.exchange.programId, this.publicKey);
    ...
    async deposit(amount) {
        ...
        if (this._insuranceDepositAccount === null) {
            ...
            tx.add(await instructions.initializeInsuranceDepositAccountIx(this.publicKey, this.publicKey, this.whitelistInsuranceAccountAddress));
        }
        tx.add(instructions.depositInsuranceVaultV2Ix(amount, this._insuranceDepositAccountAddress, this._usdcAccountAddress, this.publicKey));
    ...
    async withdraw(percentageAmount) {
        let tx = new web3_js_1.Transaction();
        tx.add(instructions.withdrawInsuranceVaultV2Ix(percentageAmount, this._insuranceDepositAccountAddress, this._usdcAccountAddress, this.publicKey));
        let txId = await utils.processTransaction(this._provider, tx);
