/* eslint-disable no-console */ /* eslint-disable unicorn/no-null */ /* eslint-disable @typescript-eslint/prefer-nullish-coalescing */ /* eslint-disable @typescript-eslint/no-non-null-assertion */ import { ComputeBudgetProgram, Connection, PACKET_DATA_SIZE, PublicKey, Transaction, TransactionMessage, VersionedTransaction } from "@solana/web3.js";
import bs58 from "bs58";
import { buildJitoTipInstruction } from "./jito.mjs";
/**
 ...
 */ export const PACKET_DATA_SIZE_WITH_ROOM_FOR_COMPUTE_BUDGET = PACKET_DATA_SIZE - 52;
/**
 ...
 */ export function getSizeOfTransaction(instructions, versionedTransaction = true, addressLookupTable) {
 ...
 */ export function getSizeOfCompressedU16(n) {
 ...
 */ export class TransactionBuilder {
        ...
        if (this.transactionInstructions.length === 0) {
            ...
            const sizeWithComputeUnits = getSizeOfTransaction([
                ...
                this.transactionInstructions.length % JITO_BUNDLE_SIZE === 0 // This transaction may be the first of a Jito bundle, so we leave room for a Jito tip transfer.
                 ? buildJitoTipInstruction(this.payer, 1) : ComputeBudgetProgram.setComputeUnitPrice({
                    microLamports: 1
                }),
   ...
   */ async buildVersionedTransactions(args) {
        ...
        return this.transactionInstructions.map(({ instructions, signers, computeUnits }, index)=>{
            ...
            if (computeUnits > DEFAULT_COMPUTE_BUDGET_UNITS * instructions.length || args.tightComputeBudget) {
                instructionsWithComputeBudget.push(ComputeBudgetProgram.setComputeUnitLimit({
                    units: computeUnits
                }));
            }
            if (args.computeUnitPriceMicroLamports) {
                instructionsWithComputeBudget.push(ComputeBudgetProgram.setComputeUnitPrice({
                    microLamports: args.computeUnitPriceMicroLamports
                }));
            }
            if (args.jitoTipLamports && index % JITO_BUNDLE_SIZE === 0) {
                instructionsWithComputeBudget.push(buildJitoTipInstruction(this.payer, args.jitoTipLamports));
            }
            ...
            for (const instruction of instructionsWithComputeBudget){
                ...
                if (sizeWithInstruction > PACKET_DATA_SIZE) {
                    if (instructionsToSend.length === 0) {
                        throw new Error(`An instruction is too big to be sent in a transaction (${sizeWithInstruction.toString()} > ${PACKET_DATA_SIZE.toString()} bytes)`);
                    }
                    break;
                }
                instructionsToSend.push(instruction);
   ...
   */ static batchIntoLegacyTransactions(instructions, priorityFeeConfig) {
        const transactionBuilder = new TransactionBuilder(PublicKey.unique(), new Connection("http://placeholder.placeholder")); // We only need wallet and connection for `VersionedTransaction` so we can put placeholders here
 ...
 */ export async function sendTransactions(transactions, connection, wallet, maxRetries) {
    ...
    for (const [index, transaction] of transactions.entries()){
        ...
        const txSignature = bs58.encode(isVersionedTransaction(tx) ? tx.signatures[0] || new Uint8Array() : tx.signature ?? new Uint8Array());
