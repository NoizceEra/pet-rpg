...
# Setup Logging
LOG_FILE = os.path.join(os.path.dirname(__file__), "fastloop.log")
...
def discover_fast_market_markets(asset="BTC", window="5m"):
    ...
    # Sort by soonest expiring (end_time ascending)
    url = (
...
def _parse_fast_market_end_time(question):
    ...
    if not match:
        print(f"DEBUG: No pattern match for {question}")
        return None
    try:
        ...
        # print(f"DEBUG: Parsed {question} as {dt} (UTC)")
        return dt
    except Exception:
        return None
...
def find_best_fast_market(markets):
    ...
    for m in markets:
        ...
        # Filter: Must be > MIN_TIME (e.g. 60s) AND < 12 hours (skip tomorrow's daily markets)
        if remaining > MIN_TIME_REMAINING and remaining < 43200:
