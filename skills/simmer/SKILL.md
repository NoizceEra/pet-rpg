...
```bash
curl.exe -X POST https://api.simmer.markets/api/sdk/agents/register \
...
```bash
curl.exe https://api.simmer.markets/api/sdk/agents/me \
...
```bash
curl.exe https://api.simmer.markets/api/sdk/markets \
  -H "Authorization: Bearer $SIMMER_API_KEY"
```
...
```bash
curl.exe -H "Authorization: Bearer $SIMMER_API_KEY" \
...
curl.exe "https://clob.polymarket.com/midpoint?token_id=TOKEN_ID"

# Price history (hourly, last week)
curl.exe "https://clob.polymarket.com/prices-history?market=TOKEN_ID&interval=1w&fidelity=60"

# Your Polymarket positions (not $SIM)
curl.exe "https://data-api.polymarket.com/positions?user=YOUR_WALLET_ADDRESS"

# Your PnL / leaderboard stats
curl.exe "https://data-api.polymarket.com/v1/leaderboard?user=YOUR_WALLET_ADDRESS&timePeriod=ALL"
```
