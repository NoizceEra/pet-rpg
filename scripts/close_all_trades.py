...
# Close ALL active positions across venues
positions = simmer_request("/api/sdk/positions").get("positions", [])
for p in positions:
    if p.get("status") == "active":
        print(f"Closing position: {p['question']} ({p.get('currency')})")
        if p.get('shares_yes', 0) > 0.001:
            res = simmer_request("/api/sdk/trade", "POST", {
                "market_id": p['market_id'],
                "side": "yes",
                "action": "sell",
                "shares": p['shares_yes'],
                "venue": p.get("venue", "polymarket")
            })
            print(f"Sell YES result: {res}")
        if p.get('shares_no', 0) > 0.001:
            res = simmer_request("/api/sdk/trade", "POST", {
                "market_id": p['market_id'],
                "side": "no",
                "action": "sell",
