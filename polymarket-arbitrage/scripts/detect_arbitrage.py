...
def main():
    ...
    try:
        # Instead of calling pinchie_alerts.py directly (which may fail due to 401),
        # we log to a special alerts file that the main agent heartbeats on.
        alert_file = 'polymarket-arbitrage/alerts_staging.json'
        with open(alert_file, 'a') as af:
            for arb in all_arbs[:1]:
                 alert_data = {
                     "timestamp": datetime.utcnow().isoformat(),
                     "title": arb['title'],
                     "url": arb['url'],
                     "type": arb['type'],
                     "entry_price": f"${arb['probabilities'][0]/100:.3f}",
                     "profit_pct": f"{arb['net_profit_pct']:.2f}%"
                 }
                 af.write(json.dumps(alert_data) + "\n")
    except Exception as e:
        print(f"Alerting staging failed: {e}", file=sys.stderr)
