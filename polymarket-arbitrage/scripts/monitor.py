...
def monitor_loop(interval, min_edge, alert_webhook, data_dir):
    ...
    while True:
        ...
        fetch_cmd = f"python {script_dir}/fetch_markets.py --output {markets_file}"
        run_command(fetch_cmd, "Fetching markets...")
        
        # Step 2: Detect arbitrage
        detect_cmd = f"python {script_dir}/detect_arbitrage.py {markets_file} --min-edge {min_edge} --output {arbs_file}"
...
def main():
    ...
    if args.once:
        ...
        run_command(f"python {script_dir}/fetch_markets.py --output {markets_file}")
        run_command(f"python {script_dir}/detect_arbitrage.py {markets_file} --min-edge {args.min_edge} --output {arbs_file}")
